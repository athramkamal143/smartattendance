...
export default function App() {
  ...
  const handleDeleteStudent = async (id: string) => {
    if (!confirm("Are you sure you want to delete this student from the records?")) return;
    try {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 dark:bg-slate-950 dark:text-slate-100 flex flex-col font-sans transition-colors duration-200">
      ...
      {printReportMeta && (
        <div id="print-canvas" className="hidden print:block p-8 bg-white text-black min-h-screen">
          <div className="text-center border-b-2 border-slate-300 pb-4 mb-6">
            <h1 className="text-3xl font-extrabold tracking-tight text-blue-900">CAMPUS ATTENDANCE GENERAL LEDGER</h1>
            <h2 className="text-xl font-medium text-slate-700 mt-2">{printReportMeta.title}</h2>
            <p className="text-sm text-slate-500 mt-1">Report Generated: {printReportMeta.date}</p>
          </div>
          ...
          <div className="mt-12 flex justify-between text-xs text-slate-500 border-t pt-4">
            <p>Smart Attendance college management tool v1.0. All values are calculated in live database.</p>
                  ...
                  {activeTab === "students" && (
                    <div className="space-y-6">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 md:col-span-1 space-y-4">
                        ...
                        <table className="min-w-full border-collapse text-left text-sm">
                          <thead>
                            <tr className="bg-slate-50 dark:bg-slate-800 border-b border-slate-100 dark:border-slate-800 text-xs text-slate-700 dark:text-slate-100 uppercase font-semibold whitespace-nowrap">
                              ...
                              <th className="p-4 text-right">Reset Key</th>
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-slate-100 dark:divide-slate-800 whitespace-nowrap">
                            {faculty.map((facItem) => (
                              <tr key={facItem.id} className="hover:bg-slate-50/50">
                                ...
                                <td className="p-4 text-right">
                                  <button
                                    ...
                                    className="px-2 py-1 border dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800 text-xs rounded"
                                  >
                                    Reset Pass
                                  </button>
                                </td>
                  ...
                  <div className="bg-gradient-to-r from-blue-700 to-indigo-800 dark:from-slate-900 dark:to-slate-950 p-6 sm:p-8 rounded-3xl text-white shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    <div className="flex space-x-2">
                      <button
                        ...
                        className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all ${!selectedAssignment ? "bg-white text-blue-900 font-bold" : "bg-white/10 hover:bg-white/20"}`}
                      >
                        Assigned Register
                      </button>
                    </div>
